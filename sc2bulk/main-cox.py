...
sc_PredDF = Predict(model, bulk_GPmat=bulk_gene_pairs_mat, sc_GPmat=single_cell_gene_pairs_mat,
                    mode=mode, sc_rownames=scAnndata.obs.index.tolist(), do_reject=True, tolerance=0.02)

scAnndata = categorize(scAnndata, sc_PredDF, do_cluster=False)

from collections import Counter
Counter(scAnndata.obs["Rank_Label"])

df_p_value = Pcluster(scAnndata = scAnndata, clusterColName = "Cell_subtype", perm_n = 1001)
df_p_value.to_csv(os.path.join(savePath,"Celltype P-value df.csv"))
