...
def plot_score_distribution_(savePath,suffix):
    savePath_3 = os.path.join(savePath, "3_Analysis")

    ## Load data
    f = open(os.path.join(savePath_3, "saveDF_bulk_"+suffix+".pkl"), "rb")
    bulk_PredDF = pickle.load(f)
    f.close()

    f = open(os.path.join(savePath_3, "saveDF_sc_"+suffix+".pkl"), "rb")
    ...
    plt.savefig(
        os.path.join(savePath_3, "TiRank Pred Score Distribution_"+suffix+".png"),
        bbox_inches="tight",
        pad_inches=1,
    )
    plt.show()
