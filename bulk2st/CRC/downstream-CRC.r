...
slicesPath <- paste0(dataPath, "slices/")
savePath <- "./tempfiles/"
...
results <- list()
clinical <- read.csv(paste0(dataPath, "clinical.csv"))

# for(slice_ in slices){
#     system(paste0("mv ",savePath,slice_,"/q05_cell_abundance_w_sf.csv ",slicesPath,slice_,"/"))
#     system(paste0("mv ",savePath,slice_,"/spot_predict_score.csv ",slicesPath,slice_,"/"))
# }

## Output the information of matrix
for (slice_ in slices) {
    ## Load spatial slice
    score <- read.csv(paste0(slicesPath, slice_, "/", "spot_predict_score.csv"), row.names = 1)
    deconv <- read.csv(paste0(slicesPath, slice_, "/", "q05_cell_abundance_w_sf.csv"), row.names = 1)
    patho <- read.csv(paste0(slicesPath, slice_, "/", "Pathologist_Annotations_", slice_, ".csv"), row.names = 1)
    colnames(patho) <- "Pathologist Annotation"
    ...
    p1 <- SpatialDimPlot(stAnndata, pt.size.factor = 1.2) + ggtitle("Gene cluster Label")
    ...
    p3 <- SpatialDimPlot(stAnndata, pt.size.factor = 1.2) + ggtitle("scRank Label")
    p3_ <- DimPlot(stAnndata) + ggtitle("Pathology Label")

    if (!dir.exists(paste0(figurePath, slice_, "/"))) {
        dir.create(paste0(figurePath, slice_, "/"), recursive = T)
    }
    pdf(paste0(figurePath, slice_, "/", "clusters spatial plot.pdf"), height = 6, width = 15)
    print(p1 + p2 + p3)
    dev.off()

    pdf(paste0(figurePath, slice_, "/", "clusters spatial plot on UMAP.pdf"), height = 6, width = 15)
    print(p1_ + p2_ + p3_)
    dev.off()

    ## Rank score feature plot

    # p <- SpatialFeaturePlot(stAnndata, features = "Rank_Score", pt.size.factor = 1) +
    ...
    # savePathTemp <- paste0(paste0(figurePath, slice_, "/", "celltypes_fraction/"))
...
head(combined_data)
# write.csv(combined_data,paste0("/home/lenislin/Experiment/projects/scRankv2/bulk2st/data_for_downstream.csv"))
combined_data <- read.csv(paste0("/home/lenislin/Experiment/projects/scRankv2/bulk2st/data_for_downstream.csv"))

combined_data <- Match_Pathology_label(df = df, merge_rules = merge_rules)
table(combined_data["Pathologist.Annotation"])
table(combined_data["Pathologist.Annotation2"])
...
if (1) {
    ...
    freqTable <- table(combined_data$Pathologist.Annotation, combined_data$Rank_Label)
    df <- as.data.frame(freqTable)
    df <- df[df[, 1] != "", ]

    # merge label
    colnames(df) <- c("Pathologist.Annotation", "scRank_label", "Freq")
    df <- Match_Pathology_label(df = df, merge_rules = merge_rules)

    # Calculating cluster totals and proportions
    clusterTotals <- aggregate(Freq ~ Pathologist.Annotation, data = df, sum)
    df <- merge(df, clusterTotals, by = "Pathologist.Annotation")
    ...
    #     sub_df <- subset(df, Pathologist.Annotation == region)
    ...
    # Order
    colnames(df) <- c("Region", "RankLabel", "Number", "Region2", "TotalNumber", "Proportion")
    df <- df[order(df$Region2, df$Region), ]
    orderedClusters <- unique(df$Region)
    df$Region <- factor(df$Region, levels = orderedClusters)

    # df$NormalizedEntropy <- entrodf[match(df$Region,rownames(entrodf)),1]

    # Plot
    plot <- ggplot(data = df, aes(x = Region, y = Proportion, fill = RankLabel)) +
    ...
    # Saving the plot
    ggsave(paste0(figurePath, "Rank label distribution in he region.pdf"), plot, width = 6, height = 8)
}
...
if (1) {
    ...
    pdf(paste0(figurePath, "SPP1 and Fibroblast of histology group.pdf"), height = 8, width = 10)
    print(p)
    dev.off()
}
...
if (1) {
    ...
    for (slice_ in slices) {
        ...
        pdf(paste0(figurePath, slice_, "/", "Interesting Region.pdf"), height = 6, width = 8)
        print(p)
        dev.off()
        ...
        pdf(paste0(figurePath, slice_, "/", "Interesting Region Rank score featureplot.pdf"), height = 6, width = 15)
        print(p1 + p2 + p3)
        dev.off()

        p4 <- SpatialFeaturePlot(stAnndata_, features = "Rank_Score", alpha = c(0,0))+theme(legend.position = "None")
        pdf(paste0(figurePath, slice_, "/", "Interesting Region HE plot.pdf"), height = 6, width = 6)
        print(p4)
        dev.off()
    }
}
...
if (1) {
    ...
    VolcanoPlot(result_df,
        pthreshold = 0.05, fcthreshold = 1.15,
        filename = paste0(figurePath, "Volcano of Celltypes fraction difference of Rank_pos versus Rank_neg.pdf")
    )
}
...
if (1) {
    ...
    col_idx <- match(c("Sample", "Pathologist.Annotation", types1, types2), colnames(combined_data))
    ...
    pdf(paste0(figurePath, "Types abundance correlation in scRank+ of given region.pdf"), height = 8, width = 8)
    corrplot(tdc,
...
if (1) {
    ...
    pdf(paste0(figurePath, "test.pdf"), height = 18, width = 4)
    print(p)
    dev.off()
}
...
if (1) {
    ...
    for (slice_ in slices) {
        stAnndata <- all_slice_seurat[[slice_]]
        # stAnndata
        savePath1 <- paste0(figurePath, slice_, "/")
...
if (1) {
    ...
    for (i in 1:length(compare_groups)) {
        ...
        pdf(paste0(figurePath, "Abundance difference of group ", group_name, ".pdf"), height = 25, width = 15)
        print(p)
        dev.off()
    }
}
...
if (1) {
    ...
    for (i in 1:length(compare_groups)) {
        ...
        pdf(paste0(figurePath, "Risk Score difference of group ", group_name, ".pdf"), height = 6, width = 4)
        print(p)
        dev.off()
    }
}
...
if (1) {
    ...
    for (i in 1:length(compare_groups)) {
        ...
        for (ranklabel in c("scRank-", "scRank+")) {
            ...
            pdf(paste0(figurePath, ranklabel, " fraction difference of group ", group_name, ".pdf"), height = 6, width = 4)
            print(p)
            dev.off()
        }
    }
...
if (1) {
    ...
    pdf(paste0(figurePath, "Rank score and types abundance correlation.pdf"), height = 15, width = 20)
    corrplot(tdc,
        method = "color", col = addcol(100),
        tl.col = "black", tl.cex = 0.8, tl.srt = 45, tl.pos = "lt",
        p.mat = testRes$p, diag = T, type = "upper",
...
if (1) {
    ...
    pdf(paste0(figurePath, "Risk Label Counts difference of histology group.pdf"), height = 4, width = 12)
    print(p)
    dev.off()
}
